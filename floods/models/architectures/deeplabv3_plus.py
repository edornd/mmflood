...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        ...
        if not pretrained or in_channels != 3:
            self.layer0 = nn.Sequential(nn.Conv2d(in_channels, 64, 7, stride=2, padding=3, bias=False),
...
def assp_branch(in_channels, out_channles, kernel_size, dilation):
    ...
    return nn.Sequential(
        ...
        nn.BatchNorm2d(out_channles), nn.ReLU(inplace=False))
...
class ASSP(nn.Module):
    def __init__(self, in_channels, output_stride):
        ...
        self.avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), nn.Conv2d(in_channels, 256, 1, bias=False),
